...
static rx_handler_result_t handle_frame( struct sk_buff **pskb ) {
   ...
   struct iphdr *ip_header = ip_hdr(skb);//(struct iphdr *)skb->network_header;
   ...
   if (ip_header->protocol==17) {
      if( (child!=NULL)) {
         ...
         LOG("Protocol UDP, from %d to %d to iface", src_port, dest_port);
...
static netdev_tx_t start_xmit( struct sk_buff *skb, struct net_device *dev ) {
   ...
   unsigned int src_port = 0;
   unsigned int dest_port = 0;
   if ( ip_header->protocol==17 && child != NULL ) {
      ...
      src_port = (unsigned int)ntohs(udp_header->source);
      dest_port = (unsigned int)ntohs(udp_header->dest);
      LOG("Protocol UDP, from %d to %d out of iface", src_port, dest_port);
